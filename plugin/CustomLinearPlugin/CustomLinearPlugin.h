...
// ---------------------------------------------------------------------------------
namespace {
static const char *PLUGIN_NAME{"CustomLinear"};
...
public:
  linearPlugin(const std::string &name, Weights weight_q, Weights bias_q)
      : name_(name) {
    WHERE_AM_I();
    n_ = weight_q.count;
    assert(n_ < 1280);
  ...
  IPluginV2DynamicExt *clone() const noexcept override {
    WHERE_AM_I();
    linearPlugin *p = new linearPlugin(name_, weight_q_, bias_q_);
...
public:
  ...
  IPluginV2 *createPlugin(const char *name,
    ...
    return new linearPlugin(name, w_q, b_q);
  }

  IPluginV2 *deserializePlugin(const char *name, const void *serialData,
                               size_t serialLength) noexcept override {
    return new linearPlugin(name, serialData, serialLength);
  }
